...
const assetsVersion = require('./assets-version')
...
`,
  ...
  assetsVersionChecker: assetsVersion(fs, path, semver),
  validateLocalAssets: () => {
    const localPackagePath = path.join(process.cwd(), 'package.json')
    if (impl.fileExists(localPackagePath)) {
      // Check for local assets version conflict
      const versionResult = impl.assetsVersionChecker.checkLocalAssetVersion(process.cwd())
      if (versionResult !== assetsVersion.SAME_VERSION) {
        const localVersion = impl.assetsVersionChecker.localAssetVersion(process.cwd())
        const defaultVersion = impl.assetsVersionChecker.defaultAssetVersion()

        throw new Error([
          `${os.EOL}\tYour local function project assets in ${process.cwd()} implements a different`,
          `${os.EOL}\tversion (${localVersion}) than expected by this version of Serverless Artillery (${defaultVersion}).`,
          `${os.EOL}\t`,
          `${os.EOL}\tTo continue to use this version of SA, please run the "configure" command`,
          `${os.EOL}\tin a clean directory and then customize your worker's assets.`,
        ].join(''))
      }
    }
  },
